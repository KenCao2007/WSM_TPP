...
from Utils import thp_loglikelihood, eval_accuracy # sahp_score_loglikelihood
import matplotlib.pyplot as plt
from preprocess.Dataset import get_dataloader
from transformer.Models import thp_Transformer, ithp_Transformer, get_non_pad_mask
...
def train(model, training_data, validation_data, optimizer, scheduler, pred_loss_func, opt):
    ...
    best_tll = thp_loglikelihood(model, validation_data, opt)
    ...
    if epoch_i < eval_after:
        ...
        # if epoch_i % 10 == 0:
        #     # torch.save(model.state_dict(), opt.model_saved_path + opt.model_saved_name + '.pth')
        #     torch.save(model.state_dict(), opt.results_saved_path + opt.model_saved_name + '.pth') # save a copy to results folder
        #     logging.info("saving models to {}".format(opt.results_saved_path + opt.model_saved_name + '.pth'))
...
def main():
    ...
    parser.add_argument('-data', type=str, default="data/retweet/")
    ...
    parser.add_argument('-method', type=str, choices=["wsm","mle","dsm"], default='mle')
    parser.add_argument('-T_per_batch', type=bool, default=True) # T are set to be the maximum event time at each batch.
    parser.add_argument('-model', type=str, choices=['thp_transformer', "ithp_transformer", "sahp"], default='sahp')
    ...
    # parser.add_argument('-h_type', type=str, default="two_side_op", choices=['None','two_side_op','two_side_ord','one_side'])
    ...
    parser.add_argument('-seq_trunc', type=int, default=1)
    ...
    parser.add_argument('-eval_per_epoch', type=int, default=0) # calculate log-likelihood every epoch. Memory consuming!
    ...
    # opt.model_saved_path = 'saved_model'
    ...
    elif opt.model == "ithp_transformer":
        model = ithp_Transformer(
            num_types=num_types,
            d_model=opt.d_model,
            d_inner=opt.d_inner_hid,
            n_layers=opt.n_layers,
    ...
    if not opt.eval_per_epoch:
        opt.train_able=False
        model.load_state_dict(torch.load(opt.results_saved_path + opt.model_saved_name + '.pth'))
        tll = Utils.thp_loglikelihood(model, testloader, opt)
